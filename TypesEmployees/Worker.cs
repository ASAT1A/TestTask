...
namespace TestTask
{
    // Наследуюмый класс рабочего со специальной информацией: Имя руководителя
    public class Worker : BaseEmployee
    {
        ...
        public Worker(string Name, int IdGender, int IdSubdivision, string SpecialInfo, DateTime Birthday) : base(Name, IdGender, IdSubdivision, SpecialInfo, Birthday)
        {
        ...
        public override void PrintInfo(int positionX, int positionY)
        {
            ...
            Print.PrintDoubleText("Имя руководителя: ", SpecialInfo, positionX, positionY + 5, ConsoleColor.Gray, ConsoleColor.White);
        }
        ...
        public override void SetSpecialInfo(int positionX, int positionY, int countLines)
        {
            ...
            while (endSpecialInfo)
            {
                ...
                Print.PrintText("Введите ФИО руководителя подразделения: ", positionX, positionY + 3, ConsoleColor.White);
